...
const App: React.FC = () => {
  ...
  const [weeklyLayout, setWeeklyLayout] =
    useState<WeeklyLayout>("weekdayFocus");
  ...
  const handleConnectCalendar = useCallback(async (url: string) => {
    ...
    try {
      ...
      const formattedEvents: CalendarEvent[] = calendarData.events
        .map((event: any) => ({
          id: event.uid,
          title: event.summary,
        ...
        .concat(
          calendarData.occurrences.map((occurrence: any) => ({
            id: `${occurrence.item.uid}-${occurrence.startDate
              .toJSDate()
              .toISOString()}`,
      ...
      alert(
        "Could not load calendar. Please check the URL and ensure it's a valid iCal/.ics link."
      );
  ...
  return (
    <div
      ...
      style={{
        background: "linear-gradient(135deg, #f9f9f9 0%, #f8f5f8 100%)",
      }}
    >
      {isLoading && (
        <div className="fixed inset-0 bg-white bg-opacity-75 flex items-center justify-center z-50">
          <div className="animate-spin rounded-full h-16 w-16 border-t-2 border-b-2 border-sakura-pink"></div>
        </div>
      )}
      ...
      <header className="p-4 md:px-8 md:py-6 flex justify-between items-center">
        <div className="flex-shrink-0 h-8 flex items-center">
          {logo ? (
            ...
            <h1 className="text-xl md:text-2xl font-bold text-gray-800 underline decoration-sakura-pink/40 decoration-1 underline-offset-4">
              Printable Calendar
            </h1>
          )}
        </div>
        <div className="flex items-center gap-2 md:gap-4">
          ...
          {viewType === "weekly" && (
            <div className="flex items-center bg-gray-200 rounded-lg p-0.5">
              ...
              >
                <span className="text-base" role="img" aria-label="target">
                  🎯
                </span>
              </button>
              <button
              ...
              >
                <span className="text-base" role="img" aria-label="island">
                  🏝️
                </span>
              </button>
            </div>
          ...
          <div className="relative">
            <select
              ...
              className="bg-white shadow-sm text-gray-900 text-sm rounded-lg focus:ring-sakura-pink focus:border-sakura-pink block w-full py-1.5 pl-3 pr-8 appearance-none"
              aria-label="Select print range"
            >
              {[...Array(12)].map((_, i) => (
                <option key={i + 1} value={i + 1}>
                  {i + 1} {viewType === "monthly" ? "Month" : "Week"}
                  {i > 0 ? "s" : ""}
                </option>
              ))}
            </select>
          ...
          <button
            ...
            className="flex items-center gap-2 bg-lipstick-red text-white font-semibold py-1.5 px-4 rounded-lg hover:opacity-90 hover:shadow-sakura-pink/20 hover:shadow-md transition-all text-sm shadow-sm"
      ...
      >
        <div
          className={`bg-paper-white shadow-xl h-full w-full max-w-xs flex flex-col absolute top-0 right-0 transition-transform duration-300 ease-in-out ${
