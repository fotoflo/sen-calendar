    ...
    <div id="preview-container" className="space-y-8 flex flex-col items-center">
      {[...Array(printRange).keys()].map((i) => {
        ...
        return (
          <div 
            key={i}
            className={`printable-calendar-page relative bg-paper-white rounded-sm p-8 flex flex-col ${viewType} ${paperSize}`}
          >
            {viewType === 'monthly' ? (
              <MonthlyView date={pageDate} events={events} logo={logo} />
            ) : (
              <WeeklyView date={pageDate} events={events} layout={weeklyLayout} logo={logo} />
            )}
          </div>
        );
      })}
